...
__interrupt void adca_isr(){
  ...
  //Aquisição de dados para a identificação da planta
          if(SPWM_State ==2 ){

               cont++;


              if(cont>=36000){ // So entra aqui depois de 3s depois da rampa (Preciosismo meu)


                  if((cont%101==0 && index<700)|| (wa>1600 && index<700 && cont%53==0 )){
                      index++;
                  }
                  else{
                      index = index;     //trava o Buffer.
                      //Comando_L_D=0;     //Buffer cheio, desliga bancada
                  }
                   buffer[index]= wa;
